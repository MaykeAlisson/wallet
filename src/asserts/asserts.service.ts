...
export class AssertsService {
  constructor(
    ...
    private readonly repository: AssertsRepository,
  ) {}
  ...
  private async existAssert(name: string, walletId: number, id?: number) {
    const result = await this.repository.findByNameAndWallet(name, walletId);
    if (result) {
  ...
  private async findByIdAndWallet(id: number, walletId: number) {
    const result = await this.repository.findByIdAndWallet(id, walletId);
    if (!result)
